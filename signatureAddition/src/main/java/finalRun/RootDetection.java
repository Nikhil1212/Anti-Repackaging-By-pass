	...
	//	lockUnlockPhone("1234", deviceId[2]);
		...
		while(scanner.hasNext())
		{
			try
			{
				...
				String logPathRooted="/home/nikhil/Documents/apps/logs/"+packageName+"/rooted.txt";
				...
				Process process=CommandExecute.commandExecutionSh(RootDetection.pathToLS+" "+originalApkDirectory);
				...
				if(count>1)
				{
					...
					installationCommandRealDevice=LogAnalysis.pathToadb+" -s "+ deviceId[2]+" install-multiple -g "+ apkPaths;
				}
				else
				{
					installationCommandRealDevice=LogAnalysis.pathToadb+ " -s "+ deviceId[2]+ " install -g "+apkPaths;
				}
				takeDumpOnDevice(deviceId[2], deviceIdSynonym[2],installationCommandRealDevice,count,packageName,dumpPathDirectory,originalApkDirectory);
				
				String pidOriginal=FetchProcessIdDumpsys.retrievePID(packageName,deviceId[2]);
				
				LogAnalysis.usingLogcat(packageName, pidOriginal, logPathRooted, deviceId[2]);
