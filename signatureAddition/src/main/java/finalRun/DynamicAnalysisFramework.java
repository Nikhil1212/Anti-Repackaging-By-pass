 ...
 *
 */
public class DynamicAnalysisFramework {
	...
	//	lockUnlockPhone("1234", deviceId[2]);
		...
		while(scanner.hasNext())
		{
			...
			//	if(count1<1100)
				...
				Process process=CommandExecute.commandExecutionSh(DynamicAnalysisFramework.pathToLS+" "+originalApkDirectory);
				...
				takeDumpOnDevice(deviceId[2], deviceIdSynonym[5],installationCommandRealDevice,count,packageName,dumpPathDirectory,originalApkDirectory);
				
				String pidOriginal=FetchProcessIdDumpsys.retrievePID(packageName,deviceId[2]);
				
				LogAnalysis.usingLogcat(packageName, pidOriginal, logPathHooking, deviceId[2]);
